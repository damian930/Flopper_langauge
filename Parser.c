...
Array_init        (Expr*, Expr_p_array, expr_p_array_init);
Array_delete      (Expr_p_array, expr_p_array_delete);
Array_add_by_value(Expr*, Expr_p_array, expr_p_array_add_by_value);

Array_init        (Token, Token_array, token_array_init);
Array_delete      (Token_array, token_array_delete);
Array_add_by_value(Token, Token_array, token_array_add_by_value);
    ...
    switch (token.type) {
        ...
        case Token_Type_Identifier: {
            ...
            if (lexer_match_token(lexer, (int) '(')) {
                Expr_p_array expr_arr = expr_p_array_init();

                if (lexer_match_token(lexer, (int) ')') == false) {
                    do {
                        Expr* args_expr = expression(lexer);
                        expr_p_array_add_by_value(&expr_arr, args_expr);
                    } while(lexer_match_token(lexer, (int)','));
                    lexer_match_token(lexer, (int) ')');
                }

                primary = &(Primary) {
                    .type = Token_Type_Function_Call_For_Parser,
                    .union_.func_call = (Func_call) {
                        .name           = token,
                        .args_as_expr_p = expr_arr,
                    }
                };
            }
    ...
    switch (expr->type) {
        ...
        case Expr_type_primary: {
            switch (expr->union_.primary.type) {
                ...
                case Token_Type_Function_Call_For_Parser: {
...
Stmt function_declaration(Lexer* lexer) {
    ...
    Token_array arg_names = token_array_init();
    Token_array arg_types = token_array_init();
    if (lexer_peek_next_token(lexer).type != (int) ')') {
        do {
            ...
            Token var_type = lexer_consume_token__exits(lexer, Token_Type_Identifier, "Was expecting a variable type int, other are not supported. \n");

            token_array_add_by_value(&arg_names, var_name);
            token_array_add_by_value(&arg_types, var_type);
...
void stmt_delete(Stmt* stmt) {
    switch (stmt->type) {
        ...
        case Stmt_type_func_decl: {
            Stmt_scope* scope = &stmt->union_.func_decl.scope;
            Stmt stmt         = stmt_scope_to_stmt(scope);
            stmt_delete(&stmt);

            token_array_delete(&stmt.union_.func_decl.arg_names_as_tokens); 
            token_array_delete(&stmt.union_.func_decl.arg_types_as_tokens); 
